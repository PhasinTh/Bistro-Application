...
import { QRCodeByBistroRequest, QRCodesRequestRequest } from './dtos/request.dto';

@Injectable()
export class AppService {
  constructor(@InjectRepository(QRCode) private qrcodeRepository: Repository<QRCode>) { }
  ...
  async findByBistro(data: QRCodeByBistroRequest) {
    try {
      return paginate<QRCode>(this.qrcodeRepository, { limit: data?.meta?.limit , page: data?.meta?.page}, {
        where: {
